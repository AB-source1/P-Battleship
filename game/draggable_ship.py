...
# ─── Map only the sizes you generate (SHIP_SIZES = [5,4,3] for GRID≤12) ───
SHIP_IMAGE_FILES = {
...
class DraggableShip:
    def __init__(self, size: int, x: int, y: int):
        ...
        # ─── Scale as vertical (upright) base ────────────────────────────────
    ...
    def rotate(self):
        """
        Toggle orientation and swap image appropriately.
        """
    ...
    def place(self, coords: list[tuple[int, int]]):
        """Final placement of ship on grid."""
        self.coords = coords
